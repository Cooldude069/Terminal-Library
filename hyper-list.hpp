...
public:
    ...
    List(long length = 1, bool limited = false) // Default constructer.
    {
        this->length = length;
    ...
    List &extend(long extensionBy)
    {
        ...
        */
        if (length + extensionBy > limit && limit != 0L)
        {
            throw std::out_of_range("List exceeds the length of the list");
        }
